...
export function doLayerReset(layer, force) {
  if (!force) {
    if (!LAYERS[layer].canPrestige()) return;

    const playerData = layerExist(layer)
      ? layerPlayer(layer)
      : setupLayer(layer);
    playerData.did = true;
  }
...
function recordLayerStats(toReset, layer, force) {
  ...
  if (!force) {
    const record = {
      ...
      ...LAYERS[layer].statsRecord.reset()
    };
