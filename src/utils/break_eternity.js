...
export default class Decimal {
  ...
  get m() {
    ...
    if (this.layer === 0)
      return (
        this.sign *
        (this.mag === 5e-324 ? 5 : powerOf10(Math.floor(Math.log10(this.mag))))
      );
    if (this.layer === 1)
      return this.sign * Math.pow(10, this.mag - Math.floor(this.mag));
    return this.sign;
  }
    ...
    return numItems
      ...
      .mul(
        priceStart
          .add(currentOwned.mul(priceAdd))
          .mul(2)
          .plus(numItems.sub(1).mul(priceAdd))
      );
  }
...
            */
  ...
  toString() {
    ...
    if (this.mag === Infinity || this.layer === Infinity)
  ...
  toStringWithDecimalPlaces(places) {
    if (this.layer === 0) {
      ...
      return `${decimalPlaces(this.m, places)}e${decimalPlaces(
        this.e,
        places
      )}`;
      if (this.layer === 1)
        ...
        return `${this.sign === -1 ? "-" : ""}(e^${this.layer})${decimalPlaces(
          this.mag,
          places
        )}`;
      }
  ...
  add(value) {
    ...
    if (layera === 1 && layerb === 0) {
      if (Math.abs(a.mag - Math.log10(b.mag)) > MAX_SIGNIFICANT_DIGITS)
        return a;
      const mantissa =
        b.sign + a.sign * Math.pow(10, a.mag - Math.log10(b.mag));
      return FC(
        Math.sign(mantissa),
        1,
