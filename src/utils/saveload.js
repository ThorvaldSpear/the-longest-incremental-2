...
function decompress(save) {
  let s = "";
  s = JSON.parse(LZString.decompressFromUTF16(save));
  if (!s) s = JSON.parse(LZString.decompressFromBase64(save));
  return s;
}
/**
...
export function downloadSave() {
  try {
    const blob = new Blob([compress()], { type: "text/plain" });
    const url = URL.createObjectURL(blob);

    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = "save.tli2";
    anchor.click();
