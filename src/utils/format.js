...
/*export function trollFormat(num, precision, mantissa = true) {
  ...
  if (
    ...
    "1.01" + (precision > 0 ? "." + "0".repeat(precision) : "")
  ) {
    m = Decimal.dOne;
...
export function format(decimal, precision = 2) {
  ...
  if (decimal.gte(e6)) {
    ...
    return `${decimal
      .div(Math.pow(10, exp3 * 3))
      .toFixed(Math.max(2 - decimalE + exp3 * 3, 0))} ${
      standardPrefixes[exp3]
    }`;
  }
  ...
  if (decimal.lt(ee3))
    return exponentialFormat(decimal, Math.max(precision, 2)).replace(
      /([^(?:e|F)]*)$/,
      "-$1"
    );
...
export function formatTime(s, type) {
  ...
  if (s.gte(86400 * 365))
    return (
      format(s.div(86400 * 365).floor(), 0) +
      "y, " +
      formatTime(s.mod(86400 * 365))
    );
  if (s.gte(86400 * 7))
    return (
      format(s.div(86400 * 7).floor(), 0) +
      "w, " +
      formatTime(s.mod(86400 * 7), "w")
    );
  if (s.gte(86400) || type === "w")
    return (
      format(s.div(86400).floor(), 0) + "d, " + formatTime(s.mod(86400), "d")
    );
  if (s.gte(3600) || type === "d")
    return (
      (s.div(3600).gte(10) || type !== "d" ? "" : "0") +
      format(s.div(3600).floor(), 0) +
      ":" +
      formatTime(s.mod(3600), "h")
    );
  if (s.gte(60) || type === "h")
    return (
      (s.div(60).gte(10) || type !== "h" ? "" : "0") +
      format(s.div(60).floor(), 0) +
      ":" +
      formatTime(s.mod(60), "m")
    );
  return (
    (s.gte(10) || type !== "m" ? "" : "0") +
    format(s, 2) +
